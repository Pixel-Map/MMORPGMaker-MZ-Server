            ...
            switch (_a.label) {
                case 0:
                    if (req.body === undefined || (req.body.password && req.body.username) === undefined) {
                        return [2 /*return*/, res.status(403).send({ message: 'Fields missing' })];
                    }
                    return [4 /*yield*/, mmoCore.database.findUser(req.body)];
                case 1:
                    user = _a.sent();
                    if (user === undefined) {
                        return [2 /*return*/, res.status(500).send({ message: "User doesn't exist" })];
                    }
                    // If password is incorrect.
                    if (security.hashPassword(req.body.password.toLowerCase()) !== user.password.toLowerCase()) {
                        return [2 /*return*/, res.status(500).send({ message: 'Incorrect password' })];
                    }
                    // If permission is incorrect
                    if (user.permission < 100) {
                        return [2 /*return*/, res.status(500).send({ message: 'You are not permitted to use this page.' })];
                    }
...
router.get('/logout', security_1.isTokenValid, function (req, res) {
    // We filter the variables to get ride of the bad one
    security_1.activeTokens[req.token.decoded.username] = security_1.activeTokens[req.token.decoded.username].filter(function (value) {
