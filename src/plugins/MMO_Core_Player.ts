...
class Core_Player {
    ...
    setCharacterImage = Game_Actor.prototype.setCharacterImage;
    setFaceImage = Game_Actor.prototype.setFaceImage;
    partyInit = Game_Party.prototype.initialize;
    ...
    updateSkin(payload) {
        if (payload.type === 'sprite') {
            this.Player['skin']['characterName'] = payload.characterName;
            this.Player['skin']['characterIndex'] = payload.characterIndex;
        }
...
DataManager.setupNewGame = function () {
    ...
    $gamePlayer.reserveTransfer(
        MMO_Core_Player.Player['mapId'],
        MMO_Core_Player.Player['x'],
        MMO_Core_Player.Player['y'],
    );
...
Game_Actor.prototype.setup = function (actorId) {
    ...
    // Use ENS name by default, unless they don't have one, then use username.

    const playerName = MMO_Core_Player.Player['ens']
        ? MMO_Core_Player.Player['ens']
...
Scene_Map.prototype.onMapLoaded = function () {
    ...
    const players = {}; // Reinit the player variable
...
Game_Switches.prototype.setValue = function (switchId, value) {
    if (switchId > 0 && switchId < $dataSystem.switches.length) {
        MMO_Core.socket.emit('player_global_switch_check', {
            switchId: switchId,
            value: value,
        });
...
Game_Variables.prototype.setValue = function (variableId, value) {
    if (variableId > 0 && variableId < $dataSystem.variables.length) {
        MMO_Core.socket.emit('player_global_variables_check', {
...
export default MMO_Core_Player;
