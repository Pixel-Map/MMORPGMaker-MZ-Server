...
exports.initialize = function (mmoCore: MMO_Core) {
    ...
    io.on('connect', function (player) {
        player.on('party_player_join_fight', async function (gameTroop) {
            ...
            const partyMembers = (await socket.modules.player.subs.player.getPlayers(player.isInParty)) || {};
            ...
            for (const k in partyMembers) {
                ...
                partyMembers[k].emit(
                    ...
                    exports.combat[player.isInParty][player.playerData.mapId].state,
                );
            }
        ...
        player.on('party_player_action_fight', async (action) => {
            ...
            if (actionsLength === membersLength) {
                ...
                exports.combat[player.isInParty][player.playerData.mapId].initiator.emit(
        ...
        player.on('party_player_estimate_fight', function (results) {
            ...
            for (const k in exports.combat[player.isInParty][player.playerData.mapId].members) {
                exports.combat[player.isInParty][player.playerData.mapId].members[k].emit(
                    'party_player_action_fight',
                    results,
                );
            }
    ...
    exports.joinParty = async (joiner, joinee) => {
        ...
        const maxMembers = gamedata.data.Actors.length - 1; // We take the maximum party members set in RPG Maker MV
        const partyName = `party-${joinee.id}`; // We prepare the party name
        const rawPartyMembers = (await socket.modules.player.subs.player.getPlayers(partyName)) || {};
    ...
    exports.disbandParty = async (partyName) => {
        const partyMembers = await socket.getConnectedSockets(partyName);
        const partyLeader = await socket.modules.player.subs.player.getPlayerById(partyName.split('party-')[1]);
    ...
    exports.refreshPartyData = async (partyName) => {
        ...
        for (const k in rawPartyMembers) {
            ...
            partyMembers[k].isInitiator = !!(
                exports.combat[partyName][rawPartyMembers[k].playerData.mapId] &&
