...
(function (m) {
  ...
  var A = O.prototype,
    ...
    getPlotBandPath: function (a, b, c) {
      var d = this.center,
          ...
          f,
          k = this.isCircular;
      this.options.gridLineInterpolation === "polygon" ? d = this.getPlotLinePath(a).concat(this.getPlotLinePath(b, !0)) : (a = Math.max(a, this.min), b = Math.min(b, this.max), k || (j[0] = this.translate(a), j[1] = this.translate(b)), j = T(j, function (a) {
  ...
  w(z, "getLabelPosition", function (a, b, c, d, g, e, j, l, h) {
    ...
    f.isRadial ? (a = f.getPosition(this.pos, f.center[2] / 2 + r(e.distance, -25)), e.rotation === "auto" ? d.attr({
      rotation: i
    }) : k === null && (k = f.chart.renderer.fontMetrics(d.styles.fontSize).b - d.getBBox().height / 2), s === null && (f.isCircular ? (this.label.getBBox().width > f.len * f.tickInterval / (f.max - f.min) && (o = 0), s = i > o && i < 180 - o ? "left" : i > 180 + o && i < 360 - o ? "right" : "center") : s = "center", d.attr({
      align: s
    ...
    i.columnrange = v(i.arearange, {
      ...
      translate: function () {
        ...
        t(b.points, function (a) {
          ...
          Math.abs(s) < o ? (o -= s, s += o, i -= o / 2) : s < 0 && (s *= -1, i -= s);
  ...
  i.waterfall = v(i.column, {
    ...
    translate: function () {
      ...
      for (c = 0, a = d.length; c < a; c++) {
        ...
        if (g.isSum) g.y = G(l);else if (g.isIntermediateSum) g.y = G(l - f);
        j = P(h, h + g.y) + k[0];
    ...
    processData: function (a) {
      ...
      for (k = 0; k < g; k++) f = b[k], d = c && c[k] ? c[k] : {}, f === "sum" || d.isSum ? b[k] = G(j) : f === "intermediateSum" || d.isIntermediateSum ? b[k] = G(e) : (j += f, e += f), l = Math.min(j, l), h = Math.max(j, h);
  ...
  O.prototype.beforePadding = function () {
    ...
    t(this.series, function (b) {
      var g = b.options;
      if (b.bubblePadding && (b.visible || !c.options.chart.ignoreHiddenSeries)) if (a.allowZoomOutside = !0, n.push(b), e) t(["minSize", "maxSize"], function (a) {
        var b = g[a],
            e = /%$/.test(b),
            b = B(b);
        h[a] = e ? f * b / 100 : b;
    ...
    t(n, function (b) {
      ...
      if (m > 0) for (; f--;) I(c[f]) && a.dataMin <= c[f] && c[f] <= a.dataMax && (h = b.radii[f], d = Math.min((c[f] - l) * p - h, d), g = Math.max((c[f] - l) * p + h, g));
    });
